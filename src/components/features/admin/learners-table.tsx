...
import { ArrowDown, ArrowUp, ArrowUpDown, Ban, Building2, Check, ChevronLeft, ChevronRight, Eye, GraduationCap, KeyRound, Pencil, ShieldOff, Trash2 } from "lucide-react";
...
function UserMenu({ user, disabled, run, ask, assign }: { user: AdminUserRow; disabled: boolean; run: (action: () => Promise<ActionResult>) => void; ask: (title: string, description: string, action: () => Promise<ActionResult>) => void; assign: (kind: "company" | "course") => void }) {
  return <AdminActionMenu triggerLabel={`Actions pour ${user.name ?? user.email}`} menuLabel={`Menu d’actions pour ${user.name ?? user.email}`}>
    {(close) => <>
      <MenuLink href={`/admin/utilisateurs/${user.id}`} icon={<Eye className="h-4 w-4" />} onSelect={close}>Voir le profil</MenuLink>
      <MenuLink href={`/admin/utilisateurs/${user.id}`} icon={<Pencil className="h-4 w-4" />} onSelect={close}>Modifier</MenuLink>
      <MenuButton icon={<GraduationCap className="h-4 w-4" />} onClick={() => { close(); assign("course"); }}>Inscrire à une formation</MenuButton>
      <MenuButton icon={<Building2 className="h-4 w-4" />} onClick={() => { close(); assign("company"); }}>Affecter à une société</MenuButton>
      <MenuButton disabled={disabled} icon={<KeyRound className="h-4 w-4" />} onClick={() => { close(); ask("Réinitialiser le mot de passe ?", "Toutes les sessions ouvertes seront invalidées. Le nouveau mot de passe ne sera affiché qu’une fois.", async () => { const form = new FormData(); form.set("userId", user.id); return resetCenterAccountPassword({ success: false }, form); }); }}>Réinitialiser le mot de passe</MenuButton>
      {user.status === "ACTIVE" && !user.bannedAt ? <MenuButton disabled={disabled} icon={<ShieldOff className="h-4 w-4" />} onClick={() => { close(); ask("Suspendre ce compte ?", "Son accès sera immédiatement interrompu.", () => suspendUser(user.id, "Action administrateur depuis la liste")); }}>Suspendre</MenuButton> : <MenuButton disabled={disabled} icon={<Check className="h-4 w-4" />} onClick={() => { close(); run(() => reactivateUser(user.id)); }}>Activer</MenuButton>}
      <MenuButton disabled={disabled} icon={<Ban className="h-4 w-4" />} onClick={() => { close(); ask("Bannir ce compte ?", "La personne ne pourra plus se connecter.", () => banUser(user.id, "Action administrateur depuis la liste")); }}>Bannir</MenuButton>
      <MenuButton danger disabled={disabled} icon={<Trash2 className="h-4 w-4" />} onClick={() => { close(); ask("Supprimer ce compte ?", "Une demande de suppression RGPD sera enregistrée.", () => deleteUserGdpr(user.id)); }}>Supprimer</MenuButton>
    </>}
  </AdminActionMenu>;
}
...
function MenuLink({ href, icon, children, onSelect }: { href: string; icon: React.ReactNode; children: React.ReactNode; onSelect: () => void }) { return <Link role="menuitem" href={href} onClick={onSelect} className="flex items-center gap-2 rounded-md px-2.5 py-2 hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{icon}{children}</Link>; }
function MenuButton({ icon, children, danger, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { icon: React.ReactNode; danger?: boolean }) { return <button role="menuitem" type="button" className={`flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-40 ${danger ? "text-destructive" : ""}`} {...props}>{icon}{children}</button>; }
