...
import { BadgeCheck, CalendarPlus, GraduationCap, RotateCcw, Search, SlidersHorizontal, UserRoundPlus, UsersRound } from "lucide-react";
...
import { getAdminUsersDashboardStats, listAdminUsers, listUserCountries, type AdminUsersSort } from "@/server/queries/admin-users";

export const metadata: Metadata = { title: "Apprenants — CRM admin" };
export const dynamic = "force-dynamic";

interface UserSearchParams {
  q?: string; role?: UserRole | "ALL"; companyId?: string; status?: AccountStatus;
  country?: string; page?: string; pageSize?: string; sort?: AdminUsersSort; direction?: "asc" | "desc";
}
...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<UserSearchParams> }) {
  ...
  const hasFilters = Boolean(params.q || params.companyId || params.status || params.country || (params.role && params.role !== "ALL"));
  ...
  return <div className="flex h-full max-h-[calc(100dvh-12.5rem)] min-h-0 flex-col gap-3 overflow-hidden" data-testid="learners-workspace">
    <header className="flex shrink-0 items-center justify-between gap-4">
      <div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 ring-1 ring-inset ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300"><UsersRound className="h-5 w-5" /></span><div className="flex min-w-0 items-baseline gap-3"><h1 className="truncate text-2xl font-semibold tracking-tight">Gestion des apprenants</h1><span className="whitespace-nowrap text-sm text-muted-foreground">{stats.total.toLocaleString("fr-FR")} comptes</span></div></div>
      <LearnerHeaderActions companies={companies} courses={publishedCourses} trainingCenter={trainingCenter} />
    </header>

    <section className="grid shrink-0 grid-cols-2 overflow-hidden rounded-xl border border-border/75 bg-card shadow-sm sm:grid-cols-3 lg:grid-cols-5" aria-label="Statistiques des apprenants">
      <CompactStat icon={<UsersRound className="h-4 w-4" />} label="Total des comptes" value={stats.total} tone="bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-300" />
      <CompactStat icon={<GraduationCap className="h-4 w-4" />} label="Élèves" value={stats.students} tone="bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300" />
      <CompactStat icon={<UserRoundPlus className="h-4 w-4" />} label="Formateurs" value={stats.instructors} tone="bg-sky-50 text-sky-700 dark:bg-sky-500/10 dark:text-sky-300" />
...
function UserFilters({ params, companies, countries, hasFilters }: { params: UserSearchParams; companies: Array<{ id: string; name: string }>; countries: Array<{ country: string; count: number }>; hasFilters: boolean }) {
  return <form className="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-border/75 bg-card p-2.5 shadow-sm 2xl:flex-nowrap" role="search"><div className="relative min-w-64 flex-[2_1_20rem]"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input name="q" aria-label="Rechercher par nom, prénom ou e-mail" defaultValue={params.q ?? ""} placeholder="Nom, prénom ou e-mail…" className="h-9 pl-9" /></div><Select name="companyId" defaultValue={params.companyId ?? ""} aria-label="Société" className="h-9 min-w-36 flex-1 py-1"><option value="">Société · Toutes</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</Select><Select name="role" defaultValue={params.role ?? "ALL"} aria-label="Rôle" className="h-9 min-w-32 flex-1 py-1"><option value="ALL">Rôle · Tous</option><option value="STUDENT">Élèves</option><option value="INSTRUCTOR">Formateurs</option><option value="ADMIN">Administrateurs</option><option value="MODERATOR">Modérateurs</option><option value="SUPPORT">Support</option><option value="FINANCE">Finance</option><option value="MANAGER">Gestionnaires</option></Select><Select name="status" defaultValue={params.status ?? ""} aria-label="Statut" className="h-9 min-w-32 flex-1 py-1"><option value="">Statut · Tous</option><option value="ACTIVE">Actifs</option><option value="PENDING_VERIFICATION">En attente</option><option value="SUSPENDED">Suspendus</option><option value="DELETED">Supprimés</option></Select><Select name="country" defaultValue={params.country ?? ""} aria-label="Pays" className="h-9 min-w-32 flex-1 py-1"><option value="">Pays · Tous</option>{countries.map(({ country }) => <option key={country} value={country}>{country}</option>)}</Select><Button type="submit" variant="outline" size="sm" className="h-9"><SlidersHorizontal className="h-4 w-4" />Filtres</Button>{hasFilters ? <Button asChild variant="ghost" size="icon" className="h-9 w-9"><Link href="/admin/utilisateurs" aria-label="Réinitialiser les filtres" title="Réinitialiser"><RotateCcw className="h-4 w-4" /></Link></Button> : null}</form>;
}
