...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<UserSearchParams> }) {
  ...
  return <div className="flex h-full max-h-[calc(100dvh-12.5rem)] min-h-0 flex-col gap-3 overflow-hidden" data-testid="learners-workspace">
    <header className="flex shrink-0 items-center justify-between gap-4">
      ...
      <LearnerHeaderActions companies={companies} courses={publishedCourses} trainingCenter={trainingCenter} exportFilters={filters} />
    </header>
...
function UserFilters({ params, companies, countries, hasFilters }: { params: UserSearchParams; companies: Array<{ id: string; name: string }>; countries: Array<{ country: string; count: number }>; hasFilters: boolean }) {
  return <form className="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-border/75 bg-card p-2.5 shadow-sm 2xl:flex-nowrap" role="search"><div className="relative min-w-64 flex-[2_1_20rem]"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input name="q" aria-label="Rechercher par nom, prénom ou e-mail" defaultValue={params.q ?? ""} placeholder="Nom, prénom ou e-mail…" className="h-9 pl-9" /></div><Select name="companyId" defaultValue={params.companyId ?? ""} aria-label="Société" className="h-9 min-w-36 flex-1 py-1"><option value="">Société · Toutes</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</Select><Select name="status" defaultValue={params.status ?? ""} aria-label="Statut" className="h-9 min-w-32 flex-1 py-1"><option value="">Statut · Actifs et suspendus</option><option value="ACTIVE">Actifs</option><option value="PENDING_VERIFICATION">En attente</option><option value="SUSPENDED">Suspendus</option><option value="DELETED">Archivés</option></Select><Select name="country" defaultValue={params.country ?? ""} aria-label="Pays" className="h-9 min-w-32 flex-1 py-1"><option value="">Pays · Tous</option>{countries.map(({ country }) => <option key={country} value={country}>{country}</option>)}</Select><Button type="submit" variant="outline" size="sm" className="h-9"><SlidersHorizontal className="h-4 w-4" />Filtres</Button>{hasFilters ? <Button asChild variant="ghost" size="icon" className="h-9 w-9"><Link href="/admin/utilisateurs" aria-label="Réinitialiser les filtres" title="Réinitialiser"><RotateCcw className="h-4 w-4" /></Link></Button> : null}</form>;
}
