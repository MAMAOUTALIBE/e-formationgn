...
async function serve(request: Request, context: { params: Promise<{ id: string; resourceId: string }> }, headOnly: boolean) {
  ...
  // Type dérivé du fichier réellement déposé, jamais de la colonne : celle-ci
  // vient du client et n'était pas revalidée à l'écriture. Un `text/html`
  // déclaré sur un dépôt accepté aurait été servi tel quel depuis l'origine de
  // la plateforme, donc exécuté comme du code de la page. Même règle que pour
  // les ressources de leçon (`/api/lecons/.../ressource/...`).
  const contentType = resourceUploadContentType(resource.storageUrl, "");
  ...
  if (resource.storageUrl.startsWith("/uploads/")) {
    ...
    return streamStoredFile({ filePath, contentType, request, headOnly, cacheControl: CACHE, downloadName: download ? resource.title : undefined });
  }
  ...
  const headers = new Headers({ "content-type": contentType, "cache-control": CACHE, "x-content-type-options": "nosniff" });
